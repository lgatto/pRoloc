...
proteomics data. This document is currently under still construction;
please see the \Rpackage{pRoloc} tutorial for details about the
package itself.
...
For a general practical introduction to \Rpackage{pRoloc}, readers are
...
that can be readily used with \Rpackage{pRoloc}. The data set can be
listed with \Rfunction{pRolocdata} and loaded with the
\Rfunction{data} function. Each data set, including its origin, is
individually documented.

The data sets are distributed as \Robject{MSnSet} instances. Briefly,
these are dedicated containers for quantitation data as well as
feature and sample meta-data. More details about \Robject{MSnSet}s are
available in the \Rpackage{pRoloc} tutorial and in the
\Rpackage{MSnbase} package, that defined the class.
...
the \Rfunction{plot2D} function and the \Rpackage{MLInterfaces}
package \cite{MLInterfaces}. The former takes an \Robject{MSnSet}
...
\Rpackage{pRoloc} also implements a \Rfunction{MLean} method for
\Robject{MSnSet} instances, allowing to use the relevant
...
algorithms, we refer to the \Rpackage{pRoloc} tutorial for its usage
...
\Rpackage{class}. For each row of the test set, the $k$ nearest (in
...
package \Rpackage{caret}. Partial least square discriminant analysis
...
\Rpackage{pRoloc} makes use of the functions \Rfunction{svm} from
package \Rpackage{e1071} and \Rfunction{ksvm} from \Rpackage{kernlab}.

\paragraph{Artificial neural network (ANN)} Function \Rfunction{nnet}
from package \Rpackage{nnet}. Fits a single-hidden-layer neural
network, possibly with skip-layer connections.

\paragraph{Naive Bayes (NB)} Function \Rfunction{naiveBayes} from
package \Rpackage{e1071}. Naive Bayes classifier that computes the
...
\citep{Andersen2003}, $\chi^2$ is defined as \emph{the [summed]
  squared deviation of the normalized profile [from the marker] for
  all peptides divided by the number of data points}, i.e. $\chi^{2} =
\frac{\sum_{i=1}^{n} (x_i - m_i)^{2}}{n}$, whereas \citep{Wiese2007}
...
example the with $sigma$ of the Radial Basis Function (Gaussian
...
\citep{Dunkley2006} (\Rfunction{dunkley2006} from
\Rpackage{pRolocdata}). As highlighted in the squared region, we can
...
or our marker set as opposed to a higher number of neighbours ($k =
10$ on the right). While one could be tempted to believe that
...
\textit{over-optimised} for the marker set or, in other words, lack
generalisation to new instances. We will demonstrate this using
simulated data taken from \cite{ISLwR} and show how these
\textit{over-optimised} perform well for the data used to compute them
at the cost of new data points.

\begin{figure}[!hbt]
\centering
    \includegraphics[width=1\textwidth]{./Figures/knnboundaries.pdf}
    \caption{Classification boundaries using $k=1$ or $k=10$ on the
...
not flexible and produces nearly linear and approximate separation
boudaries (such a classifier is said to have low variance but high
...
wronly classified points ) of 0.1695 and 0.1925, respectively.

\begin{figure}[!hbt]
\centering
    \includegraphics[width=1\textwidth]{./Figures/ISL-2_16.pdf}
    \caption{The KNN classifier using $k = 1$ (left, solid
      classification boudaries) and $k = 100$ (right, solid
      classification boundaries) compared the optimal decision
      boudaries. Reproduced with permission from \cite{ISLwR}.}
\label{fig:ISL1}
\end{figure}
...
not \textit{seen} this data. If the model perform well on new data, it
is said to generalise well. This is a quality that is required in most
cases, and in particular in our organelle proteomics experiments where
the training data corresponds to our marker sets. Figure
\ref{fig:ISL2} plots the respective training and testing error rates
as a function of $1/k$ which is a reflection of the
flexibility/complexity of our model; when $k = 1$ and $1/k = 1$ (far
right), we have a very flexible model with the risk of
over-optimisation. Greater values of $k$ (towards the left)
characterise less flexible models. As can be seen, high values of $k$
produce poor performance for both training and testing data. However,
while the training error steadily decreases when the model complexity
increases (smaller $k$), the testing error rate displays a typical
U-shape: at a value around $k = 10$, the testing error rate reaches a
minimum and then start to increase due to over-optimisation to the
training data and lack of generalisation of the model.

\begin{figure}[!hbt]
\centering
    \includegraphics[width=.8\textwidth]{./Figures/ISL-2_17.pdf}
\caption{(Reproduced with permission from \cite{ISLwR}).}
\label{fig:ISL2}
\end{figure}

These results show that adequate optimisation of the model parameters
are essential to avoid either too flexible models (an this do not
generalise to new data) or models that do not describe the decision
...
\Rpackage{pRoloc}. The analysis typically consists of two steps. The
...
used to infer the subcellular location of the unlabelled examples.
...
The result of the optimisation procedure are stored in an
\Robject{GenRegRes} object that can be inspected, plotted and best
...
distributed in the \Rpackage{pRolocdata package}. On figure
