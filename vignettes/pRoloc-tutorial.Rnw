...
accessing the feature variable of interested and the \Rcode{table}
function, such as \Rcode{table(fData(tan2009r1)\$PLSDA)}.
...
the experiment under investigation, while the original markers
published with the data can be found in the slot
\texttt{"markers.orig"}).
...
mandatory to highlight other annotation, such as to visualise the set
of markers described in the original publication.
...
@ 
\caption{Reduced set of markers for the \Robject{tan2009r1} data
...
\begin{figure}[h]
  ...
  \caption{Snapshot of the 3-dimensional PCA plot of the hyperLOPIT
    experiment from \cite{Christoforou:2016} along the first 3
...
\paragraph{NB} It is essential to emphasise that the accuracy scores
obtained during parameter optimisation are only a reflection of
classification performance on a set of distinct and ideally separated
spatial clusters\footnote{Indeed, we assume that the data is
  ...
  fail in the absence of such separation.}. These scores are not
representative of the reliability of the final classification
(described in section \ref{sec:sml}), in particular along the
boundaries separating the different sub-cellular niches. High scores
at the optimisation stage are a requirement to proceed with the
analysis, but are by no means indicative of the false positive rate in
...
@ 
%% $
\caption{ Second round of classification using the augmented set of
  markers obtained using \Rfunction{phenoDisco} as detailed in
  \cite{Breckels2013} and a weighted svm classifier. }
\label{fig:pdres2}
\end{figure}
