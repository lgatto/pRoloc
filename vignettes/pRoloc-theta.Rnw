...
\subsection{Preparing the auxiliary data from the Go ontology}\label{sec:auxgo}
...
\subsubsection{The \Robject{dunkley2006} data}
...
\subsubsection{The \Robject{andy2011} data}
...
\subsection{A note on reproducibility}\label{sec:annotrepro}
...
\subsubsection{Choosing weights}
