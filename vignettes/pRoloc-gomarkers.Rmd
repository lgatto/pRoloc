...
Now the parameters for the search have been defined we can use the `addGoMarkers` function to add a GO information matrix to the `featureData` slot of the dataset. The `addGoMarkers` function takes a `MSnSet` instance as input (from which the `featureNames` will be extracted) and it downloads the CC terms (the default, biological process and the molecular function namespaces are also supported) found for each protein in the dataset. The output `MSnSet` has the CC term binary matrix appended to the `fData`, by default this is called `GOMarkers` (and changed using the `fcol` argument).
...
The `addGoMarkers` function by defualt does not do any filtering of the terms evidence codes unless specified in the `evidence` argument, see `?addGoMarkers` for more details. 
...
Now we have extracted and filtered annotation information for our dataset we re-order the `GOMarkers` matrix of terms according to their correlation with the dataset structure. To do this we use the `orderGoMarkers` function. 
...
5. Orders the annotation information in `GOMarkers` according to the minimum normalised Euclidean distance.
...
res <- orderGoMarkers(cc, k = 1:3, p = 1/3, verbose = FALSE)
```
...
The `getNormDist` function can be used to extract a `vector` of normalised distances. Which can then be used to rank and order the terms in the `GOMarkers` matrix, as per the code chunk below. 
...
## Re-order `GOMarkers` matrix in `fData`
