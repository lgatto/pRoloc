##' Hyper-parameters regularisation for the random forest algorithm.
##' 
...
##' as generated by \code{\link{rfRegularisation}}.
...
rfPrediction <- function(object,
  ...
  if (missing(assessRes)) {
    if (missing(mtry))
      stop("First run 'rfRegularisation' or set 'mtry' manually.")
    params <- c("mtry" = mtry)
  } else {
