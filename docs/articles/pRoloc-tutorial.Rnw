...
\bioctitle[\Biocpkg{pRoloc} tutorial]{Using \Biocpkg{pRoloc} for
...
\packageVersion{\Sexpr{BiocStyle::pkg_ver("pRoloc")}}

Report issues on \url{https://github.com/Bioconductor/pRoloc/issues}
...
retrieved from the supplementary file \texttt{pr800866n\_si\_004.xls}
(\url{http://pubs.acs.org/doi/suppl/10.1021/pr800866n/suppl\_file/pr800866n\_si\_004.xls})
...
\subsection{A shorter input work flow}

The \Rfunction{readMSnSet2} function provides a simplified import
pipeline.  It takes a single spreadsheet as input (default is
\texttt{csv}) and extract the columns identified by \Robject{ecol} to
create the expression data, while the others are used as feature
meta-data. \Robject{ecol} can be a \Robject{character} with the
respective column labels or a numeric with their indices. In the
former case, it is important to make sure that the names match
exactly. Special characters like \texttt{'-'} or \texttt{'('} will be
transformed by \R into \texttt{'.'} when the \texttt{csv} file is read
in.  Optionally, one can also specify a column to be used as feature
names.  Note that these must be unique to guarantee the final object
validity.
...
\texttt{","} for \texttt{csv}, \texttt{"\textbackslash t"} for \texttt{tsv},
...
variables called \Robject{markers.orig} and the output of the partial
least square discriminant analysis, applied in the original
publication, in the \Robject{PLSDA} feature variable. The most
up-to-date marker list for the experiment can be found in
\Robject{markers}. This feature meta-data column can be added from a
simple \texttt{csv} markers files using the \Rfunction{addMarkers}
function - see \Rfunction{?addMarkers} for details.

The organelle markers are illustrated below using the convenience
function \Rfunction{getMarkers}, but could also be done manually by
accessing feature variables directly using \Rfunction{fData()}.
...
g\section{Data visualisation}\label{sec:viz}
...
\subsection{Profile plots}\label{sec:profplot}
...
\subsection{Sub-cellular cluster dendrogram}\label{sec:dendro}
...
above. The \Rfunction{mrkHClust} calculates average class profiles and
generates the resulting dendrogram.
...
mrkHClust(tan2009r1)
@  
  \caption{Average distance between organelle classes.}
  \label{fig:dendro}
\end{figure}
...
Note the colours of the labels on the dendrogram
(figure~\ref{fig:dendro}), which match the colours used to annotate
PCA plots, described in the next section
(figure~\ref{fig:plot2d}). These colours are defined at the session
level (see \Rfunction{getStockcol} and \Rfunction{setStockcol}) and
re-used throughout \Biocpkg{pRoloc} for consistent annotation.

\subsection{Dimensionality reduction}\label{sec:pcalot}
...
feature meta-data column (slot \Robject{featureData}, accessed as a
\Robject{data.frame} with the \Robject{fData} function) that is
declared with the \Robject{fcol} argument (default is
...
can be found in the slot \texttt{"markers.orig"}).

\begin{figure*}
<<plot2d, echo=TRUE, fig.asp=1>>=
plot2D(tan2009r1, fcol = "markers")
addLegend(tan2009r1, fcol = "markers", cex = .7,
          where = "bottomright", ncol = 2)
@ 
\caption{PCA plot. Representation of the \Sexpr{nrow(tan2009r1)}
  protein of \Robject{tan2009r1} after reduction of the 4 reporter
  quantitation data to 2 principal components.}
\label{fig:plot2d}
\end{figure*}

As the default value for the \texttt{fcol} argument is
\texttt{"markers"}, it is not necessary to specify it. It is however
mandatory to specify other annotation feature variables, such as to
visualise the set of markers described in the original publication.

\begin{figure*}
<<plot2dorg, echo=TRUE, fig.asp=1>>=
plot2D(tan2009r1, fcol = "markers.orig")
addLegend(tan2009r1, fcol = "markers.orig", where = "bottomright")
@ 
\caption{PCA plot. Reduced set of markers for the \Robject{tan2009r1}
  data projected onto 2 principal components.}
\label{fig:plot2dorg}
\end{figure*}

It is also possible to visualise the data along 3 dimensions using the
\Rfunction{plot3D} function, which works like the 2 dimension version
(figure~ref{fig:plot3D}). The resulting figure can be rotated by the
user.
...
% \begin{center}
% \animategraphics[controls, loop, width=.8\linewidth]{6}{Figures/movie}{001}{040}
% \end{center}
...
\begin{figure}[h]
  ...
  \caption{Snapshot of the 3-dimensional PCA plot. The
    \Robject{tan2009r1} data is represented along the first 3 principal
    components.}
  \label{fig:plot3D}
\end{figure}
...
and~\ref{fig:plot3D}, we indicate on the axis labels the percentage of
total variance explained by the individual PCs. It is not unusual to
reach over 75\% along the first two PCs, even for experiments with
several tens of fractions. One can calculate this information for all
PCs by setting \texttt{method = "scree"} in \Rfunction{plot2D}. On
figure~\ref{fig:scree}, we see that the four PCs in the
\Robject{tan2009r1} data account for \Sexpr{paste(pcvar[1:3], collapse
  = ", ")} and \Sexpr{pcvar[4]} percent of the total variance.
...
\Rfunction{plot2D}: \Sexpr{paste(pRoloc:::plot2Dmethods, collapse = ",
  ")}. Except for \texttt{scree} (see above) and \texttt{none} (no
data transformation, which can be useful when the data is already
...
discussion here; the readers are redirected to the manual page for
more details.
...
(t-SNE)\footnote{\url{https://lvdmaaten.github.io/tsne/}}
\cite{tsne:2008} is widely applied in many areas in computational
biology and more generally field that need to visualise
high-dimensional data. The t-SNE method is non-linear, and will
emphasise separation of different features while grouping features
...
original data. See \textit{How to Use t-SNE
  Effectively}\footnote{\url{http://distill.pub/2016/misread-tsne/}}
for a useful non-technical introduction.

The results of the algorithm crucially depend on the values of its
input parameters, in particular the \textit{perplexity}, which
...
\begin{figure}[h]
  \centering
  \includegraphics{./Figures/tsne.pdf}
  \caption{Effect of t-SNE's perplexity parameter on the human
    \texttt{HEK293T2011} data.}
  \label{fig:tsne}
\end{figure}
...
with method \texttt{none} can be useful (see \texttt{?plot2D}). In the
case of this document, figure~\ref{fig:tsne}, for instance, was
pre-generated rather than computed upon compilation.
...
plot with the \Rfunction{highlightOnPlot} function. The
\Rfunction{highlightOnPlot3D} can be used to overlay data onto a 3
dimensional figure produced by \Rfunction{plot3D}.
...
\begin{figure}[h]
  \centering
  \includegraphics{./Figures/Screenshot_PCA2.png}
  \caption{Screenshot of the \Biocpkg{pRolocGUI} interface. The GUI is
    also available as an online app for the hyperLOPIT experiment from
    \cite{Christoforou:2016} at
    \url{https://lgatto.shinyapps.io/christoforou2015/}.}
  \label{fig:gui}
\end{figure}
...
\section{Assessing sub-cellular resolution}\label{sec:qsep}

TODO

%% \subsection{The sub-cellular diversity}
%% \subsection{Interpreting spatial clusters on the PCA plot}
%% \subsection{Quantitative assessment of the sub-cellular resolution}
...
\begin{figure}[!hbt]
<<plotKmeans, echo=TRUE, fig.asp=1>>=
...
\begin{figure}[!hbt]
<<plotHclust, echo=TRUE, tidy = FALSE, fig.asp=1>>=
...
demonstrate the function with only perform 10 iterations\footnote{In
  the interest of time, the optimisation is not executed but loaded
  from \Rfunction{dir(system.file("extdata", package = "pRoloc"),
    full.names = TRUE, pattern = "params.rda")}. } (parametrised with
\Rcode{times = 10}), which is enough for testing, but we recommend 100
...
\begin{figure}[!hbt]
<<params, dev='pdf', fig.width=4, fig.height=4, echo=TRUE>>=
plot(params)
levelPlot(params)
@ 
...
\warning[Important]{It is essential to emphasise that the accuracy
  scores obtained during parameter optimisation are only a reflection
  of the classification performance on a set of distinct and ideally
  separated spatial clusters. Here, we assume that the data is
  characterised by good separation of the various spatial niches which
  are reflected by sub-cellular markers. Quality control of the data
  and the markers using the visualisation described in section
  \ref{sec:viz} is essential and subsequent analyses are doomed to
  fail in the absence of such separation. \\
  These classification scores are not representative of the
  reliability of the final classification (described in section
  \ref{sec:sml}), in particular along the boundaries separating the
  different sub-cellular niches. High scores at the optimisation stage
  are a requirement to proceed with the analysis, but are by no means
  indicative of the false positive rate in the final sub-cellular
  assignment of non-marker proteins.}
...
variables (and can be defined by the \Robject{fcol} parameter)
e.g. here we use the original markers in \Robject{"markers.orig"} as a
...
automatically added to the \Robject{featureData} slot; in this case,
using the \Robject{svm} and \Robject{svm.scores} labels.

\warning[Important]{The calculation of the classification
  ...
  \textbf{not} reflect any \textbf{biologically relevant} sub-cellular
  localisation probability but rather an algorithm-specific
...
distributions. Using a unique threshold (\Robject{minprob} with value
\Sexpr{round(minprob, 2)} above) results in accepting
\Sexpr{round(sum(p2 == "ER")/sum(p1 == "ER"), 2) * 100} \% of the
initial ER predictions and only \Sexpr{round(sum(p2 ==
  "mitochondrion")/sum(p1 == "mitochondrion"), 2) * 100} \% of the
mitochondrion predictions. The \Rfunction{getPredictions} function
also accepts organelle-specific score thresholds. Below, we calculate
...
@ 
%% $
\caption{Classification results. The results of the second round of
  classification, using the augmented set of markers obtained using
  \Rfunction{phenoDisco} as detailed in \cite{Breckels2013} and a
