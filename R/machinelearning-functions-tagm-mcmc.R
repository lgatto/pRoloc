...
##'
##' The `tagmMcmcTrain` function generates the samples from the posterior distributions (object
##' or class `.MCMCParams`) based on an annotated quantitative spatial
...
tagmMcmcProcess <- function(MCMCParams
  ...
  numChains <- length(chains(myParams))
  N <- myChain@N
  K <- myChain@K
  n <- myChain@n
...
##'     MCMC algorithm. Default is 1000.
##' @param burnin The number of samples to be discarded from the begining of
##'     the chain. Default is 100.
##' @param thin The thinning frequency to be applied to the MCMC chain.
##'     Default is 5.        
...
##' @param numChains The number of parrallel chains to be run. Default it 1
...
tagmMcmcTrain <- function(object,
  ...
  .res <- bplapply(rep(numIter, numChains), FUN = tagmMcmcChain,
                   object = object,
                   fcol = fcol,
                   method = "MCMC",
                   burnin = burnin,
...
##' @param u The prior shape parameter for Beta(u, v). Default is 2
##' @param v The prior shape parameter for Beta(u, v). Default is 10.
##' @return `tagmMcmcChain` returns an instance of class [MCMCChain()].
##' @md
##' @author Oliver M. Crook
##' @seealso The [plotEllipse()] function can be used to visualise
##'     TAGM models on PCA plots with ellipses.
