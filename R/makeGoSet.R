...
##' @param id An \code{character} with feature names to be pulled
##' from biomart. If and \code{MSnSet} is provided, then
...
getGOFromFeatures <- function(id,
    ...
    if (!is.null(evidence)) {
        ...
        evidence <- match.arg(evidence, getGOEvidenceCodes(), several.ok=TRUE)
        if ("EXP" %in% evidence)
            evidence <- unique(c(evidence,"EXP", "IDA", "IPI", "IMP", "IGI", "IEP"))
        if ("ISS" %in% evidence)
            evidence <- unique(c(evidence,"ISO", "ISA", "ISM", "IGC", "IBA", "IBD", "IKR", "IRD", "RCA"))
