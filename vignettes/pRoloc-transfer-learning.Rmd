...
![Selecting species (left) and feature type (right) to create an `AnnotationParams` instance for the human `andy2011` data.\label{fig:apgui}](./Figures/ap12.png)
...
appropriate `MSnSet` object, and filter out proteins without any
interaction scores.
...
`r nrow(andy2011)` primary experimental protein profiles along a
sub-cellular density gradient (`andy2001`) and `r nrow(ppi)` auxiliary
...
argument^[Large scale applications of this algorithms were run on a cluster using an MPI backend defined with `SnowParams(256, type="MPI")`.]. Also,
in the interest of time, the weights optimisation is repeated only 5
times below.
