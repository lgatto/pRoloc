...
\warning{It is essential to emphasise that the accuracy scores
  obtained during parameter optimisation are only a reflection of the
  classification performance on a set of distinct and ideally
...
\warning{These classification scores are not representative of the
...
\warning{The calculation of the classification probabilities is
  dependent on the classification algorithm.  These probabilities are
  not to be compared across algorithms; they do \textbf{not} reflect
  any \textbf{biologically relevant} sub-cellular localisation
  probability but rather an algorithm-specific classification
  confidence score.}
