...
abstract: >
  ...
  spatial proteomics data in the `r Biocpkg("pRoloc")` package. Please
  see the `pRoloc-tutorial` vignette for details about the package
  itself.
output:
...
```{r foreword, echo=FALSE, results="asis"}
...
We recommend the book \textit{An Introduction to Statistical
  Learning} (<http://www-bcf.usc.edu/~gareth/ISL/>) by
[@ISLwR] for a more detailed introduction of machine learning.
...
measure of *exactness* -- returned output is a relevant result)
...
equivalently, *algOptimization*. See table \ref{tab:algo}
...
method by [@Breckels2013] (figure \ref{fig:pd}). It uses the
