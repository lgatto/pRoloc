...
tagmNoveltyTrain <- function(object,
    ...
    .tagmMcmcTrain(object, fcol, method, numIter, burnin, thin, mu0, lambda0,
                   nu0, S0, beta0, u, v, numChains, K_bar, BPPARAM)
...
tagmNoveltyProcess <- function(object,
    ...
    .pooledres <- tagmNoveltyChainProcess(object = object,
                                          params = chains(pooledparams)[[1]],
                                          fcol = fcol)

    ## Use pooled params to compute discovery probability
    pooledparams <- tagmMcmcProcess(pooledparams)
    tagm_newcluster_prob <- 1 - rowSums(pooledparams@summary@tagm.joint[, 1:K_markers])

    .out <- .NoveltyChains(pooledNoveltyChain = .pooledres,
                           noveltyChains = .res,
                           tagmNewclusterProb = tagm_newcluster_prob)
...
##'     The default it 5. See `tagmNoveltyTrain` for details.
##' @noRd
tagmNoveltyChain <- function(object,
                             fcol = "markers",
                             method = "MCMC",
                             numIter = 1000L,
                             burnin = 100L,
...
##' @param params An instance of class `NoveltyChains` as generated by
##'     `tagmNoveltyProcess`.
...
tagmNoveltyPredict <- function(object,
    ...
    if (any(c('tagm.phenotype', 'tagm.discovery.prob') %in% fvarLabels(object)))
        stop("Novelty detection results already present in the MSnSet.")
