...
page (\url{https://github.com/lgatto/pRoloc/wiki/Clustering-infrastructure})
...
\begin{figure*}
\centering
    \includegraphics{./Figures/knnboundaries.pdf}
...
\begin{figure*}[!hbt]
\centering
    \includegraphics{./Figures/ISL-2_16.pdf}
...
We recommend the book \textit{An Introduction to Statistical
  Learning} (\url{http://www-bcf.usc.edu/~gareth/ISL/}) by
...
\begin{figure*}[!hbt]
\centering
    \includegraphics{./Figures/F1boxplots.pdf}
...
We have developed a transfer learning framework \cite{Breckels:2016}
and applied to the analysis of spatial proteomics data, as described
in the \texttt{pRoloc-transfer-learning} vignette.
