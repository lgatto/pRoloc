...
\Robject{andy2011} and \Robject{andygoset} objects\footnote{We will
  ...
  feature variable, instead of the default \texttt{markers}. } and
...
former classes are not expected to benefit for other data source,
...
it is desirable to systematically favour the primary data (i.e. high
