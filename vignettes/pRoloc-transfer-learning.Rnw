...
\begin{abstract}
  ...
  localisations. The \emph{thetaClassification} algorithm combines a
...
described for the data at hand with the \Rfunction{thetaOptimisation}
function. For instance, for the human data modelled by the
\Robject{andy2011} and \Robject{andugoset} objects and the
...
The combination of weights to be tested can be defined in multiple
ways: one can pass a weights matrix explicitly generated with
\Rfunction{thetas} through the \Robject{th} argument; one can defined
the increment between weights using \Robject{by}; or by specifying the
number of weights to be used through the \Robject{length.out}
argument.
...
\Rfunction{thetaOptimisation} function supports parallelised execution
...
underlying architecture and alternative backends can be defined
through the \Robject{BPPARAM} argument. Also, in the interest of time,
