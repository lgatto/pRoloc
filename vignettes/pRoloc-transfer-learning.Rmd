...
Biomart server. In this section, we copy the experimental data to
`andyhpa`.
...
fd <- left_join(fData(andyhpa), hpa)
...
We now have two `MSnSet` objects containing respectively `r
nrow(andyppi)` primary experimental protein profiles along a
