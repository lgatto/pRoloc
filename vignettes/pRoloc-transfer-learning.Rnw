...
\begin{abstract}
  ...
  localisations. The \emph{knntlClassification} algorithm combines a
  \emph{primary} experimental spatial proteomics data (LOPIT, PCP,
  etc.)  and an \emph{auxiliary} dataset (for example binary data
...
data enabling to identify over 6000 proteins and discriminate numerous
sub-cellular and sub-organellar niches (\fixme{add ref}). Supervised
and semi-supervised machine learning algorithms can be applied to
assign thousands of proteins to annotated sub-cellular niches
\cite{Breckels2013,Gatto:2014}. These data constitute our main source
for protein localisation and are termed thereafter \emph{primary}
data.
...
\item define the annotation parameters based on the species and
  feature types;
\item query the resource defined in (1) and retrieve relevant terms;
\item use the terms to prepare the auxiliary data.
\end{enumerate}

We will describe how to perform these different steps, although, in
practice, steps 2 and 3 are combined into one single function. We will
demonstrate these steps using a LOPIT experiment on Human Embryonic
Kidney (HEK293T) fibroblast cells \cite{Breckels2013}, available and
documented in the \Biocexptpkg{pRolocdata} experiment package as
\Robject{andy2011}.
...
not neet to be explicitly set later on. The default parameters can be
...
\Robject{andy2011} dataset. We load the data and verify that the
feature names correspond to UniProt accessions.
...
<<andgo>>=
andygo <- getGOFromFeatures(andy2011)
head(andygo)
@
...
<<andgo2, eval=FALSE>>=
getGOFromFeatures(featureNames(andy2011))
@

We now have, in the \Robject{andygo}, the GO terms that have been
associated to the feature names identified in the \Robject{andy2011}
dataset. Instead of tuning this information into an \Robject{MSnSet}
manually, we can use the \Rfunction{makeGoSet} function\footnote{As
  above, the globally set parameters (\Robject{ap} above) will be used
  automatically (as the \Rfunction{getGOFromFeatures} function above),
  but they could also be passed explicitly through the
  \Robject{params} argument.}.
...
\Robject{andy2011} and \Robject{andygoset} objects and the
\Sexpr{length(m)} annotated sub-cellular localisations
(\Sexpr{paste(m[-1], collapse = ", ")} and \Sexpr{m[1]} \fixme{Use the
  \texttt{markers.tl} column here (as in large scale analysis), once
  added to the data object.}), we want to know how to optimally
combine primary and auxiliary data. If we look at figure
\ref{fig:andypca}, that illustrates the experimental separation of the
\Sexpr{length(m)} spatial classes on a principal component plot, we
...
plot2D(andy2011)
setStockcol(NULL)
addLegend(andy2011, where = "topright", bty = "n", cex = .7)
@  
...
\Rfunction{knnOptimisation}, we use 3 and 3.
...
\subsection{Favouring primary data}\label{sec:fav}
