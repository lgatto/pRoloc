...
The \Rpackage{pRolocGUI} application allow to explore the spatial
proteomics data using an interactive, web-based \Rpackage{shiny}
interface \citep{shiny}. The package is available in Bioconductor and
can be installed and started as follows:
...
\begin{figure}[h]
  \centering
  \includegraphics[width=.6\linewidth]{./Figures/Screenshot_PCA2.png}
