...
.isbig <- function(object, fcol, stockcol, stockpch, stockbg) {
  ...
  ## bg indices
  ii <- rep(1:ncolbg, ceiling(k))[1:nclst]
  return(list(big = big, toobig = toobig,
              nclst = nclst,
              ncol = ncol, npch = npch,
              k = k, kk = kk, jj = jj, ii = ii))
}
...
##' @param lwd A \code{numeric} defining the line width for drawing 
...
plot2D <- function(object,
                   ...
                   mirrorY = FALSE,
                   col,
                   bg,                      # add option to colour points by bg as per points documentation
                   palette = "light",       # allow users to pick between a "light" and "dark" theme palette
                   t = .3,                  # controls the degree of lightening/darkening
                   pch,
                   cex,
                   lwd,
                   index = FALSE,
  ...
  if (!missing(bg)) {
    ...
    if(is.null(palette)) 
      stop("'palette' can only be defined as 'dark' or 'light'")
    if (palette == "light")
      stockbg <-  lighten(stockcol, amount = t)
    if (palette == "dark")
      stockbg <-  darken(stockcol, amount = t)  
  }
  ...
  palette <- match.arg(palette, c("light", "dark"))
...
addLegend <- function(object,
  ...
  if ("bty" %in% names(pairlist(...))) 
    legend(where, txt, col = col, pch = pch, pt.bg = bg, pt.lwd = lwd, ...)
  else 
    legend(where, txt, col = col, pch = pch, bty = bty, pt.bg = bg, pt.lwd = lwd, ...)
  invisible(NULL)
}
