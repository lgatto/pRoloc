...
\begin{figure}[h]
  ...
  \caption{Snapshot of the 3-dimensional PCA plot. The
    \texttt{tan2009r1} data is represented along the first 3 principal
    components.}
  \label{fig:plot3D}
\end{figure}
...
\texttt{method = "scree"} in \Rfunction{plot2D}. Below, we see that
the four PCs in the \texttt{tan2009r1} data account for
\Sexpr{paste(pcvar[1:3], collapse = ", ")} and \Sexpr{pcvar[4]}
