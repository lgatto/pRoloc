##' Hyper-parameters regularisation for the naive Bayes algorithm.
##'
##' @title nb regularisation
##' @param object An instance of class \code{"\linkS4class{MSnSet}"}.
##' @param fcol The feature meta-data containing marker definitions.
##' Default is \code{markers}.
##' @param laplace The hyper-parameter. Default values are \code{seq(0, 5, 0.5)}.
...
nbRegularisation <- function(object,
  ...
  for (.times in 1:times) {
    ...
    for (.xval in 1:xval) {    
        ...
        .matrixF1 <- makeF1matrix(list(laplace = laplace))
  ...
  .hyperparams <- list(laplace = laplace)
  .design <- c(xval = xval,
               test.size = test.size,
               times = times)
...
##' as generated by \code{\link{nbRegularisation}}.
...
nbPrediction <- function(object,
  ...
  if (missing(assessRes)) {
    if (missing(laplace))
      stop("First run 'nbRegularisation' or set 'laplace' manually.")
